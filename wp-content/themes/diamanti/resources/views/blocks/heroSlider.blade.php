@if (isset($hero_slider))
    ...
    @endif">
        ...
        @if( isset($hero_slider['background_image']['url']) && $hero_slider['gradient_overlay'] )
            <div class="overlay bg-diamanti"></div>
        @endif
                        ...
                        <div class="homeHeroSlide">
                            <div class="text-{{ $slide['text_alignment'] }} {{ $slide['text_color'] }}">
                                ...
                                @if( $slide['call_to_action'])
                                  ...
                                  @if ($loop->first)
                                    <a class='btn btn-orange' href="{!! $cta['link'] !!}">{!! $cta['button_text'] !!}</a>
                                  @else
                                    <a class='btn btn-outline-orange' href="{!! $cta['link'] !!}">{!! $cta['button_text'] !!}</a>
                                  @endif
                                @endforeach
                                @endif
                            </div>
