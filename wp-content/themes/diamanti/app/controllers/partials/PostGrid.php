...
trait PostGrid
{
    ...
    private function post_grid_output($a = null)
    {
    	...
    	return [
            ...
            'title' => show_block_title($a['section_title'] ?? '', $a['text_color'] ?? '' . ' text-' . $a['heading_alignment'] ?? 'left'),
		    'color' => $a['text_color'] ?? '',
            'query' => $this->get_posts($a['feed_type'] ?? '', $a['posts_limit'] ?? '', $a['category'] ?? '', $a['post_list'] ?? '', $a['tag'] ?? ''),
            'background_image' => $a['background_image'] ?? null,
			'background_color' => get_color($a['background_color']['value'] ?? ''),
			'background_video' => $a['background_video'] ?? null,
			'classes' => $a['add_classes'] ?? null,
        ];
    }
}
