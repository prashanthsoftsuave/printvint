...
class WordKeeper_System {
	...
	private function define_admin_hooks() {
		...
		if ( (defined('DOING_AJAX') && DOING_AJAX) || isset($_GET['wc-ajax']) ) {
			...
			if(is_admin()) {
				...
				$this->loader->add_action('save_post', $plugin_admin, '_purge_post');
				$this->loader->add_action('delete_post', $plugin_admin, '_purge_post', 10);

				// Comment changes, hooks pass ID of commment
				$this->loader->add_action('comment_post', $plugin_admin, '_purge_comment', 10, 2);
				$this->loader->add_action('edit_comment', $plugin_admin, '_purge_comment', 10, 1);
				$this->loader->add_action('untrashed_comment', $plugin_admin, '_purge_comment', 10, 1);
				$this->loader->add_action('delete_comment', $plugin_admin, '_purge_comment', 10, 1);

				// Term/taxonomy changes, hooks pass ID of terms
				$this->loader->add_action('edit_terms', $plugin_admin, '_purge_term', 10, 2);
				$this->loader->add_action('delete_term', $plugin_admin, '_purge_term', 10, 4);
			}
		}
	}
	...
	private function define_public_hooks() {
		...
		$this->loader->add_action( 'shutdown', $plugin_public, 'buffer_stop', 1000 );
